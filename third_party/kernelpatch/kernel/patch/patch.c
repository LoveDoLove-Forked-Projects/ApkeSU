...
static void before_rest_init(hook_fargs4_t *args, void *udata)
{
    ...
    /*
     * Task-local storage and syscall argument discovery are optional for the
     * KPM ABI itself.  A device can omit one of their patch points while
     * still exposing a valid KPM loader.  Do not make one optional hook
     * failure suppress the bridge that provides load/list/control.
     */
    rc = task_observer();
