...
preset_t *get_preset(const char *kimg, int kimg_len)
{
    ...
    if (kimg_len < (int)sizeof(preset_t)) return NULL;
    return (preset_t *)memmem(kimg, kimg_len - sizeof(preset_t) + sizeof(magic), magic, sizeof(magic));
}
...
#define MAX_COMPILE_TIME_LEN 24
...
void print_preset_info(preset_t *preset)
{
    ...
    char compile_time_str[MAX_COMPILE_TIME_LEN];
    snprintf(compile_time_str, sizeof(compile_time_str), "%s %s", __TIME__, __DATE__);
...
static void require_native_kpm_abi(kallsym_t *symbols, char *image, int image_size)
{
    char *required[] = {
        "sukisu_kpm_load_module_path", "sukisu_kpm_unload_module",
        "sukisu_kpm_num", "sukisu_kpm_list", "sukisu_kpm_info",
        "sukisu_kpm_control", "sukisu_kpm_version",
        "sukisu_compact_find_symbol", "sukisu_super_access",
        "sukisu_super_find_struct", "sukisu_super_container_of",
    };
    for (size_t i = 0; i < sizeof(required) / sizeof(required[0]); ++i) {
        int offset = get_symbol_offset(symbols, image, required[i]);
        if (offset <= 0 || offset > image_size - 16 || offset % 4) {
            tools_loge_exit("Native GKI KPM ABI missing or invalid: %s; use a kernel built with CONFIG_KSU=y CONFIG_KPM=y\n", required[i]);
        }
    }
    tools_logi("Native GKI KPM ABI verified\n");
}
                     ...
                     bool root_key, const char **additional, extra_config_t *extra_configs, int extra_config_num)
{
    ...
    require_native_kpm_abi(&kallsym, kallsym_kimg, pimg.ori_kimg_len);

    // kpimg
    char *kpimg = NULL;
    int kpimg_len = 0;
    read_file_align(kpimg_path, &kpimg, &kpimg_len, 0x10);
    if (get_preset(kpimg, kpimg_len) != (preset_t *)kpimg || kpimg_len <= SZ_4K) {
        tools_loge_exit("invalid kpimg preset or missing entry code\n");
    }
    ...
    if (text_offset - kinfo->b_stext_insn_offset >= (1 << 27) ||
        !b((uint32_t *)(out_kernel_file.kimg + kinfo->b_stext_insn_offset), kinfo->b_stext_insn_offset, text_offset)) {
        tools_loge_exit("kpimg entry exceeds ARM64 branch range\n");
    }
